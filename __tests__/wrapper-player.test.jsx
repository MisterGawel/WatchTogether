...
describe('WrapperPlayer via wrapper', () => {
	it('renders Youtube thumbnail and player', () => {
		render(
			<WrapperPlayer link="https://www.youtube.com/watch?v=jfKfPfyJRdk" />
		);
	...
	it('renders Twitch thumbnail and player', () => {
		...
		expect(thumbnail).toHaveAttribute(
			'src',
			'https://static-cdn.jtvnw.net/previews-ttv/live_user_BDECERI-640x360.jpg'
		);
		...
		expect(iframe).toHaveAttribute(
			'src',
			'https://player.twitch.tv/?channel=BDECERI&parent=localhost'
		);
	});

	it('renders Dailymotion thumbnail and player', () => {
		render(
			<WrapperPlayer link="https://www.dailymotion.com/video/x1f6c9" />
		);
		...
		expect(thumbnail).toHaveAttribute(
			'src',
			'https://www.dailymotion.com/thumbnail/video/x1f6c9'
		);
