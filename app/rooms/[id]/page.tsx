...
import {
	...
	getShareMessage,
	renameRoom,
} from '@/components/room/params/roomManagement';
import {
	updateUserMessages,
	checkIfUserIsBanned,
	renameUser,
} from '@/components/room/params/userManagement';
...
export default function RoomPage({
	...
	const fetchAdminName = async (adminId: string) => {
		try {
			...
			console.error(
				'Erreur lors de la récupération du nom admin:',
				error
			);
			return 'Admin';
		}
	...
	useEffect(() => {
		const unsub = onAuthStateChanged(auth, async (user) => {
			if (user) {
				...
				if (isBanned) {
					alert(
						`Vous avez été banni de cette room (ID: ${user.uid})`
					);
					router.push('/');
					return;
				}
				...
				const isGuestBanned = await checkIfUserIsBanned(
					roomId,
					`guest_${guestName}`
				);
				if (isGuestBanned) {
	...
	const handleDeleteRoom = async () => {
		...
		if (
			!window.confirm('Êtes-vous sûr de vouloir supprimer cette room ?')
		) {
			return;
		}
	...
	return (
		<div
			className={`flex flex-col min-h-screen ${darkMode ? 'dark bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}
		>
			<header
				className={`px-6 py-4 shadow-md ${darkMode ? 'bg-gray-800' : 'bg-white'}`}
			>
				<div className="flex flex-col items-center justify-between mx-auto space-y-4 max-w-7xl md:flex-row md:space-y-0">
					...
					<div className="flex items-center space-x-4">
						...
						<Card
							className={`w-auto shadow-none ${darkMode ? 'bg-gray-800' : 'bg-white'}`}
						>
			...
			<div className="flex flex-col w-full min-h-screen gap-4 p-4 lg:flex-row">
				<div className="flex flex-col w-full gap-4 lg:w-3/5">
					<div
						className={`flex flex-col w-full h-full gap-4 px-4 py-4 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'}`}
					>
						<SearchBar
						...
						/>
						<Divider
							className={`mt-2 ${darkMode ? 'bg-gray-700' : 'bg-gray-200'}`}
						/>
						<Tabs
							isVertical
							classNames={{
								...
								cursor: darkMode
									? 'bg-gray-600 text-gray-100 font-bold'
			...
			{showModal && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
					<div
						className={`p-6 rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto ${darkMode ? 'bg-gray-800' : 'bg-white'}`}
					>
						...
						<div className="space-y-6">
							...
							{!communityId && isGuest && (
								<div
									className={`flex flex-col p-3 space-y-2 rounded-md ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}
								>
									<span
