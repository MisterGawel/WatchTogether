...
export const useRoomParams = (paramsPromise: Promise<any>) => {
	...
	const [resolvedParams, setResolvedParams] = useState<RoomParams | null>(
		null
	);
...
export const useRoomActions = (
	...
	const handleDeleteAllMessages = async () => {
		...
		const messagesRef = collection(
			db,
			`chats/${resolvedParams.id}/messages`
		);
	...
	const handleDeleteAllLinks = async () => {
		...
		const waitLinksRef = collection(
			db,
			`chats/${resolvedParams.id}/wait_links`
		);
		const histLinksRef = collection(
			db,
			`chats/${resolvedParams.id}/hist_links`
		);
