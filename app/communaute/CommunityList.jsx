...
export default function CommunityListPage() {
    ...
    const userCommunitiesList = communities.filter((commu) =>
		userCommunities[commu.id]
	  );
	  const otherCommunitiesList = communities.filter((commu) =>
		!userCommunities[commu.id]
	  );
  
	  setCommunities({ userCommunities: userCommunitiesList, otherCommunities: otherCommunitiesList });
	  setLoading(false);
		setLoading(false);
			...
			<div className="min-h-screen px-4 py-10 bg-gray-50">
			  ...
			  {loading ? (
				...
				<div>
				  ...
				  {communities.userCommunities && communities.userCommunities.length > 0 && (
					<div className="mb-8">
					  <h2 className="text-2xl font-semibold text-gray-800 mb-4">Mes communautés</h2>
					  <div className="grid gap-6 mx-auto md:grid-cols-2 lg:grid-cols-3 max-w-7xl">
						{communities.userCommunities.map((commu, index) => (
						  <motion.div
							key={commu.id}
						  ...
						  >
							<Card className="p-6 transition-all duration-300 border border-gray-200 shadow-sm cursor-pointer min-h-32 rounded-2xl hover:shadow-lg hover:border-gray-300">
							  ...
							  <p className="mt-2 text-sm text-gray-600 line-clamp-3">{commu.description}</p>
							</Card>
						  </motion.div>
						))}
					  </div>
				  ...
				  {communities.otherCommunities && communities.otherCommunities.length > 0 && (
					<div>
					  <h2 className="text-2xl font-semibold text-gray-800 mb-4">Autres communautés</h2>
					  <div className="grid gap-6 mx-auto md:grid-cols-2 lg:grid-cols-3 max-w-7xl">
						{communities.otherCommunities.map((commu, index) => (
						  <motion.div
							key={commu.id}
							initial={{ opacity: 0, y: 10 }}
							animate={{ opacity: 1, y: 0 }}
